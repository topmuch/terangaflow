...
export default function SmartTicketQRPage() {
  ...
  const [viewMode, setViewMode] = useState<'landing' | 'display'>('landing')

  // Kiosk mode for display view
  const kiosk = useKioskMode({
    blockShortcuts: false, // Only block when explicitly enabled
    preventZoom: false,
  ...
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'Escape' && viewMode === 'display') {
        handleBackToLanding()
      }
    }
      ...
      <div className="relative z-10">
        ...
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-bold text-emerald-500 font-mono tracking-wider">
                {station.code}
              </span>
...
function FeaturesSection() {
  const features = [
    {
      icon: Clock,
      title: 'Temps Réel 30s',
      description: 'Mises à jour automatiques toutes les 30 secondes',
      color: 'text-emerald-500',
    },
    {
      icon: Zap,
      title: 'Alertes Retard',
      description: 'Notifications instantanées de changements',
      color: 'text-amber-500',
    },
    {
      icon: QrCode,
      title: 'QR Connect',
      description: 'Scannez pour recevoir les alertes sur mobile',
      color: 'text-sky-500',
    },
    {
      icon: BarChart3,
      title: 'Analytics',
      description: 'Données de fréquentation et performances',
      color: 'text-purple-500',
    },
    {
      icon: Store,
      title: 'Marketplace',
      description: 'Services et offres des commerces locaux',
      color: 'text-rose-500',
    },
    {
      icon: Users,
      title: 'Multi-Tenants',
      description: 'Isolation complète entre gares et transporteurs',
      color: 'text-teal-500',
    },
  ]
...
interface DisplayViewProps {
  station: Station
  onBack: () => void
  kiosk: ReturnType<typeof useKioskMode>
}

function DisplayView({ station, onBack, kiosk }: DisplayViewProps) {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 min-w-0">
            <Button
              ...
              size="sm"
              onClick={onBack}
              className="shrink-0 -ml-2 hover:bg-muted/50"
              aria-label="Retour à la liste des gares"
            >
      ...
      {/* Main Content: Departures + Sidebar */}
      <main className="flex-1 flex flex-col lg:flex-row gap-4 md:gap-6 p-4 md:p-6 overflow-hidden">
        ...
        {/* Sidebar: Ads + Info (hidden on mobile, visible on lg+) */}
      ...
      {/* Mobile Ad (below table on small screens) */}
  ...
  return (
    <div className="rounded-lg border bg-card p-4 flex flex-col items-center gap-3">
      ...
      {isLoading ? (
        ...
        <img
          src={qrDataUrl}
          alt={`QR Code pour les alertes - Gare ${stationCode}`}
          className="w-40 h-40 rounded-lg"
          width={160}
          height={160}
        />
      ) : (
        <div className="w-40 h-40 rounded-lg bg-muted flex items-center justify-center text-muted-foreground text-xs">
          QR indisponible
        </div>
      )}

      <p className="text-xs text-muted-foreground text-center">
        Scannez pour recevoir les alertes de départ sur votre téléphone
      </p>
    </div>
  )
}
